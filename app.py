...
# --- BARRE LATÉRALE : GESTION DES ABSENCES ---
...
# Liste dynamique des agents selon les cases cochées
agents_actifs = []
...
if uploaded_file:
    if not agents_actifs:
        st.error("⚠️ Veuillez sélectionner au moins un agent disponible dans la barre latérale.")
    else:
        try:
            ...
            # 3. Attribution des agents (uniquement ceux cochés)
            df['Group_ID'] = df['Date_Format'] + "_" + df['Zone']
            groupes_uniques = df['Group_ID'].unique()
            
            # On utilise uniquement la liste agents_actifs pour le roulement
