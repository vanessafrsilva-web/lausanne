...
with st.sidebar:
    ...
    if up and st.button("🚀 Lancer l'Attribution"):
        try:
            ...
            c_id = next((c for c in df_ex.columns if 'id' in c.lower()), 'ID')
            ...
            for _, row in df_ex.sort_values(by=[c_date, c_heure]).iterrows():
                ...
                bloc = "Matin" if "matin" in str(row[c_statut]).lower() else ("Après-midi" if "midi" in str(row[c_statut]).lower() else None)
                ...
                if presents:
                    ...
                    for p in presents_tries:
                        ...
                        elif res_h == "⚠️ CONFLIT": agt_elu, h_finale = p, "⚠️ CONFLIT"

                temp = pd.concat([temp, pd.DataFrame([{
                    'ID': row[c_id], 'Batiment': row['Batiment'], 'Date': ds, 'Heure': h_finale, 'Agent': agt_elu, 
                    'Type': row[c_type], 'Rue': info_b['rue'], 'Statut': row[c_statut], 'Date_Sort': dt_raw
                }])], ignore_index=True)
...
with t1:
    if not st.session_state.db.empty:
        ...
        # Correction : On définit le style pour 7 colonnes affichées
        def style_row(s):
        ...
        st.dataframe(df_v[['Date', 'ID', 'Heure', 'Agent', 'Batiment', 'Type', 'Statut']].style.apply(style_row, axis=1), use_container_width=True, height=500)
...
with t3:
    if not st.session_state.db.empty:
        ...
        st.subheader("📊 Charge hebdomadaire")
        df_final['Semaine'] = df_final['Date_Sort'].dt.isocalendar().week
        df_final['Nom_Semaine'] = "Semaine " + df_final['Semaine'].astype(str)
        
        # Histogramme pour compter chaque ligne comme 1 mission
        fig = px.histogram(df_final.sort_values('Semaine'), x='Nom_Semaine', color='Agent', 
                           color_discrete_map=COULEURS, barmode='group', text_auto=True)
        st.plotly_chart(fig, use_container_width=True)
        
        st.subheader("🏠 Par bâtiment")
        st.table(df_final.groupby('Batiment').size().reset_index(name='Missions'))
