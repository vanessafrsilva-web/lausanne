...
with st.sidebar:
    ...
    if st.button("Valider l'absence"):
        nouv = pd.DataFrame([{'Agent': abs_agent, 'Date_Debut': a_deb.strftime('%d/%m/%Y'), 'Date_Fin': a_fin.strftime('%d/%m/%Y')}])
    ...
    if uploaded and st.button("🚀 Planifier le mois"):
...
with tab_cal:
    if not st.session_state.db.empty:
        ...
        sel_j = st.selectbox("Date à visualiser :", jours)
        
        cols = st.columns(3)
        for i, agt in enumerate(AGENTS):
            with cols[i]:
                st.markdown(f"<div style='text-align:center; background-color:{COULEURS[agt]}; padding:10px; border-radius:5px; font-weight:bold; color:black;'>{agt}</div>", unsafe_allow_html=True)
                ...
                if miss.empty:
                    ...
                    for _, row in miss.iterrows():
                        st.markdown(f"""
                        <div style='border-left: 5px solid gray; background-color: #f0f2f6; padding: 10px; margin-top: 5px; border-radius: 3px; color: black;'>
                            <b>{row['Heure']}</b> - {row['Batiment']}<br><small>{row['Rue']}</small>
                        </div>
                        """, unsafe_allow_html=True)
    else:
        st.info("Importez des données pour voir le calendrier.")
